...
- **計測開始時の案内ポップアップ**: 高さ／窓枠モードの開始時に、立ち位置（建物から離れて地面と対象を画角に入れる／窓の正面に正対する）をイラスト付きで案内する。背景タップまたは「はじめる」で閉じる。
- **単一計測**: 計測は1つ。再計測すると直前の計測が置き換わる。確定した高さは線の中点に数値（白カプセル＋黒文字）で常時表示。
- **面に沿うレティクル**: 純正「計測」アプリ風のレティクル（AR空間の白いリング）が、床・壁などの面に沿って傾いて表示される。①の底点選択は床（水平面）のときだけ計測ボタンを有効化し、壁などの誤選択を防ぐ。
- **ライブガイド**: 底点確定後、対象を狙っている間に、地面〜レティクル間の点線・暫定の高さ数値（cm/m）・延長線をリアルタイム表示。
- **撮影→保存・共有（3ステップ）**: ①計測 → ②撮影モード（地面・終点が画角に入ると緑✓のガイド付きでシャッター）→ ③プレビューから保存（カメラロール）／共有。撮影画像には高さ数値が合成され、地面・対象・鉛直線・数値が1枚に収まる。
- やり直し（`.waitingTarget` のとき直前の底点を破棄）。
- クリア（全結果と AR 上のマーカー・鉛直線を削除し連番リセット）。
- 計測値は永続化しない（アプリ終了で破棄。保存した写真はカメラロールに残る）。
- UI は Apple HIG 準拠（マテリアル背景・SF Symbols・カプセルボタン）。

...
- **内部テスターに共有しているのは `main` ブランチのビルド**である。配布の基準は常に `main`。
- **`main` にアップデート（PR マージ等）が入ったときは、都度ユーザーに「内部テスターのバージョンに反映するか（＝新しいビルドを上げて更新をかけるか）」を確認する。** ユーザーの指示なく勝手に配布ビルドを更新・再アップロードしない。
- 反映する場合は、ビルドを上げる前に `CURRENT_PROJECT_VERSION`（ビルド番号）を必ず +1 する（同一ビルド番号は再アップロードできないため）。バージョン名 `MARKETING_VERSION` は機能の節目でのみ更新する。
- Archive → App Store Connect へのアップロード、および App Store Connect 上の Web 操作（審査提出・公開リンク・テスター管理）はユーザーが手動で行う。Claude はコード改修・ビルド番号更新・git 操作までを担当し、手動作業は画面に沿って案内する。
